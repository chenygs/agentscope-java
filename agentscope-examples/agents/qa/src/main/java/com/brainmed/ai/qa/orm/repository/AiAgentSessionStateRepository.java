 ...
 */
@Repository
public interface AiAgentSessionStateRepository
        extends JpaRepository<AiAgentSessionStateEntity, AiAgentSessionStateEntity.AgentSessionStateId> {

    /** 按 sessionId + stateKey 查询所有项（用于列表读取，按 item_index 升序） */
    List<AiAgentSessionStateEntity> findBySessionIdAndStateKeyOrderByItemIndexAsc(
    ...
    /** 列出所有不同的 sessionId */
    @Query("SELECT DISTINCT p.sessionId FROM AiAgentSessionStateEntity p ORDER BY p.sessionId")
